...
namespace BLL.Services
{
    public class ProgrammerProfileService : IProgrammerProfileService
    {
        ...
        {
            if(id == null)
                throw new ValidationException("Invalid id of profile", "Id");
        ...
        {
            if (profileDTO == null)
                throw new ValidationException("Programmer profile is not supported by information.", "Id");
        ...
        public byte[] GenerateReport(List<ProgrammerProfileDTO> profiles)
        {
            ExcelFill fill;
            Border border;
            if (profiles == null)
                throw new ValidationException("Programmer profiles is not supported by information.", "Id");
            if (profiles.Count() == 0)
            ...
            using (ExcelPackage excelPackage = new ExcelPackage())
            {
                ...
                ExcelRange cell = sheet.Cells[$"A{firstRow}:F{profiles.Count() + firstRow}"];
                ...
                for (int i = firstRow + 1; i <= profiles.Count() + 1; i++)
                {
                    cell = sheet.Cells[$"A{i}"];
                    cell.Value = profiles[i - (firstRow + 1)].FullName;
                    cell = sheet.Cells[$"B{i}"];
                    cell.Value = profiles[i - (firstRow + 1)].Age;
                    cell = sheet.Cells[$"C{i}"];
                    cell.Value = profiles[i - (firstRow + 1)].Email;
                    cell = sheet.Cells[$"D{i}"];
                    cell.Value = profiles[i - (firstRow + 1)].Phone;
                    cell = sheet.Cells[$"E{i}"];
                    cell.Value = profiles[i - (firstRow + 1)].Address;
                    cell = sheet.Cells[$"F{i}"];
                    cell.Value = profiles[i - (firstRow + 1)].GitHub;
                }
